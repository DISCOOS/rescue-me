<?php
    ...
    function is_sms_error($status) {
       return !($status == 'Delivered (0)' || $status ==  '' || $status == 'Received by recipient');
    }
    ...
    function insert_trace_bar($missing, $collapsed = false, $output=true) {
        ...
        if($missing->sms_delivery !== null) {
            ...
            if(is_sms_error($missing->sms_error)){
                $trace['delivered']['state'] = 'fail';
                $trace['delivered']['tooltip'] = 'Delivery failure: ' . $missing->sms_error;
            } else {
