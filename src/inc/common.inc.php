<?php
    ...
    function create_ajax_response($html, $options = array(), $status = 200) {
     ...
     * @param boolean $ensure Ensure trailing directory separator
     * @return string Path
     */
    function get_path($root, $elements, $ensure = false) {

        if(is_array($elements) === false) {
            $elements = array($elements);
        }

        array_unshift($elements, rtrim($root, DIRECTORY_SEPARATOR));
